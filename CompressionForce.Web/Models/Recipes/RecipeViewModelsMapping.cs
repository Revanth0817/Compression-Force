...
namespace CompressionForce.Web.Models.Recipes
{
    public static class RecipeViewModelsMapping
    {
        ...
        public static Recipe ToDomain(AddRecipeVm vm)
            => new Recipe(vm.Code, vm.Name, vm.Parameters);

        public static Recipe ToDomain(EditRecipeVm vm)
            => new Recipe(vm.Code, vm.Name, vm.Parameters);

        public static EditRecipeVm ToEditVm(Recipe recipe, List<string> toolTypes, List<string> treatments, List<string> awc_arTypes, List<string> forceFeederRatioS1Types, List<string> forceFeederRatioS2Types, List<string> recipeTypes)
        {
            return new EditRecipeVm
            {
                Code = recipe.Code,
                Name = recipe.Name,
                Parameters = recipe.Parameters?.ToList() ?? new List<RecipeParameter>(),
